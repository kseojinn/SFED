...
OUTPUT_FILE = "pi_qwen3_Q4_512_ARC_HellaSwag_TruthfulQA_GSM8K_bench_results.csv"
...
def run_performance_bench(model_path, datasets_prompts):
    ...
    llm = Llama(
        ...
        n_threads=4,  # 라즈베리파이 특화 세팅 (CPU 스레드 4개 할당)
        n_ctx=2048,
        verbose=False
    )
    ...
    def _measure_inference(prompt_text, target_max_tokens):
        prompt_formatted = format_qwen_prompt(prompt_text) # Qwen 포맷 함수 호출
        ...
        output = llm(
            ...
            stop=["<|im_end|>"], # Qwen 전용 종료 토큰으로 변경
            stream=True
        )
        ...
        for chunk in output:
            ...
            # 라즈베리파이 CPU 부하를 줄이기 위해 토큰 수 단순 증가로 최적화
            generated_token_count += 1
            
        end_time = time.perf_counter()
        
        ttft = max(first_token_time - start_time, 1e-9)
    ...
    # 4. 웜업(Warm-up) 수행 및 콜드 스타트 TTFT 측정
    ...
    for target_max_tokens in MAX_TOKENS_LIST:
        ...
        for dataset_name, prompts in datasets_prompts.items():
            ...
            # 6. 결과 CSV 기록 (라즈베리파이 버전의 Peak_Mem_Mean/Std 유지 및 콜드 스타트 지표 추가)
            metrics = {
                "Model": model_name,
                "Dataset": dataset_name,
                "Max_Tokens": target_max_tokens,
                "Runs": NUM_RUNS,
                "Model_Load_Time (s)": round(load_time, 2),
                "Cold_Start_TTFT (s)": round(cold_start_ttft, 4),
                "TTFT_Mean (s)": round(np.mean(run_ttfts), 4),
                ...
                "Peak_Mem_Mean (MB)": round(np.mean(run_mems), 1), # 라즈베리파이용 통계
                "Peak_Mem_Std (MB)": round(np.std(run_mems), 1)    # 라즈베리파이용 통계
            }
